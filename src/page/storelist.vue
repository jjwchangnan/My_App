...
export default {
    ...
    methods: {
        ...
        updateData() {
			...
			temp_data = temp_data[this.storeinfo.storeid];
            for (let goodsId in temp_data) {
                for (let key in this.shoplist) {
                    if (goodsId == this.shoplist[key].goodsid) {
		...
		shopdata(data) {
			...
			for (const key in data) {
				if(key == this.storeinfo.storeid){
					temp = data[key]
				}
			}
			this.storeinfo.storename = temp.storename
			...
			for (const key in temp) {
				...
				_obj.name = temp[key].goodsname
				_obj.salescount = temp[key].salescount
				_obj.number = 0
				_obj.price = temp[key].price
				_obj.imgSrc = require("@/assets/img/shop_list/store_icon.jpg")
